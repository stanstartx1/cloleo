...
export default HeroSection;import React, { useEffect, useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Link } from 'react-router-dom';
import axios from 'axios';
import {
...
// Composant Catégorie avec MEGA-MENU style ivory.co.il
const CategoryMenuItem = ({ category, subcategories, allCategories, categoryImages, categorySlideTick }) => {
  const [showMegaMenu, setShowMegaMenu] = useState(false);
  const hasSub = subcategories && subcategories.length > 0;
  const Icon = getCategoryIcon(category.name);
  
  // Récupérer l'image de la catégorie (depuis banner_images ou image)
  const getCategoryImage = () => {
    const banners = category.banner_images || [];
    if (banners.length > 0) {
      const idx = (categorySlideTick + category.id) % banners.length;
      let img = banners[idx];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {hasSub && <ChevronRightIcon className="w-3.5 h-3.5 flex-shrink-0 ml-2 text-slate-400 group-hover:text-orange-500" />}
      </Link>
      
      {/* MEGA-MENU au survol - style ivory.co.il */}
      {hasSub && showMegaMenu && (
        <div className="absolute left-full top-0 ml-1 w-[420px] bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50">
          {/* En-tête avec le nom de la catégorie */}
            ...
            <div className="grid grid-cols-2 gap-2">
              {subcategories.map(sub => (
                <Link
                  key={sub.id}
                  to={`/categories/${sub.slug}`}
                  className="flex items-center gap-2 py-2 px-3 text-sm text-slate-600 hover:bg-orange-50 hover:text-orange-600 rounded-lg transition-colors group"
                >
                  <div className="w-5 h-5 rounded bg-slate-100 group-hover:bg-orange-100 flex items-center justify-center">
                    <ChevronRightIcon className="w-3 h-3 text-slate-400 group-hover:text-orange-500" />
                  </div>
          ...
          {/* Image mise en avant de la catégorie - style ivory.co.il */}
          {categoryImage && (
            <div className="border-t border-slate-100 p-3 bg-slate-50">
              <Link to={`/categories/${category.slug}`} className="block relative rounded-xl overflow-hidden group/image">
                <img 
                  src={categoryImage} 
                  alt={category.name}
                  className="w-full h-28 object-cover transition-transform duration-300 group-hover/image:scale-105"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent flex items-end p-3">
                  <span className="text-white text-xs font-bold">Découvrir {category.name} →</span>
                </div>
              </Link>
            </div>
          )}
          ...
          <div className="border-t border-slate-100 p-2 bg-white">
            <Link 
              to={`/categories/${category.slug}`}
              className="flex items-center justify-center gap-1 text-xs font-semibold text-orange-600 hover:text-orange-700 py-1.5 transition-colors"
            >
...
const HeroSection = ({ categories = [] }) => {
  ...
  // Animation pour l'image des catégories
  useEffect(() => {
  ...
  return (
    <section className="relative w-full bg-slate-50">
      ...
      {/* Contenu principal - Version responsive */}
      <div className="relative z-10 w-full max-w-screen-xl mx-auto px-4 py-4 md:py-6">
        
        {/* Version Desktop : 3 colonnes côte à côte */}
        <div className="hidden lg:grid lg:grid-cols-[280px_minmax(0,1fr)_280px] gap-0 items-stretch">
          
          {/* COLONNE GAUCHE : CATÉGORIES AVEC MEGA-MENU (Desktop amélioré) */}
          <div className="bg-white rounded-l-2xl shadow-lg overflow-hidden border border-slate-100 flex flex-col h-full">
            <div className="bg-gradient-to-r from-orange-500 to-amber-500 px-4 py-3 flex items-center gap-2 flex-shrink-0">
              <ShoppingBag className="w-5 h-5 text-white" />
              <span className="font-bold text-white text-sm">Toutes les catégories</span>
        ...
        {/* Version Mobile : stack vertical (inchangée) */}
        <div className="lg:hidden space-y-4">
