...
- [x] Structure modulaire créée:
  ...
  │   └── auth.py        # JWT & dépendances
  ├── models/
  │   └── schemas.py     # Modèles Pydantic
  ├── routes/
  │   └── reviews.py     # Module avis (premier module externe)
  └── server.py          # App principale avec import dynamique
  ```
- [x] JWT_SECRET stocké dans .env (partagé entre modules)
- [x] Premier routeur externe intégré avec `app.include_router()`
