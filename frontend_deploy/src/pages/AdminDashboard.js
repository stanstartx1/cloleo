...
const AdminDashboard = () => {
  ...
  const [authPageEnabled, setAuthPageEnabled] = useState(false);
  const [authPageBackgroundType, setAuthPageBackgroundType] = useState('color');
    ...
    switch (activeSection) {
      ...
      case 'settings-layout':
        return <AuthPageSettingsSection 
          enabled={authPageEnabled}
          setEnabled={setAuthPageEnabled}
          backgroundType={authPageBackgroundType}
          setBackgroundType={setAuthPageBackgroundType}
          ...
          onUploadImage={uploadAuthPageBgImage}
...
const AuthPageSettingsSection = ({ enabled, setEnabled, backgroundType, setBackgroundType, bgColor, setBgColor, bgImages, setBgImages, layoutType, setLayoutType, loading, saving, uploading, onUploadImage, onRemoveImage, onSave, getImageUrl, token, API }) => {
  return (
                  ...
                  <div
                    ...
                    style={{
                      background: backgroundType === 'color' && bgColor
                        ? bgColor
                        : backgroundType === 'image' && bgImages.length === 1
                        ? `url(${getImageUrl(bgImages[0])}) center/cover`
                        : backgroundType === 'image' && bgImages.length === 2 && layoutType === 'split'
                ...
                {backgroundType === 'image' && (
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700">
                      ...
                      {bgImages[0] ? (
                        <div className="space-y-3">
                          <img src={getImageUrl(bgImages[0])} alt="Gauche" className="w-full h-32 object-cover rounded-lg" />
                          <div className="flex gap-2">
                            <input
                              type="text"
                              value={bgImages[0]}
                        ...
                        <div>
                          ...
                          >
                            {uploading ? 'Upload en cours...' : '📁 Choisir l\'image de gauche'}
                          </label>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700">
                      ...
                      {bgImages[1] ? (
                        <div className="space-y-3">
                          <img src={getImageUrl(bgImages[1])} alt="Droite" className="w-full h-32 object-cover rounded-lg" />
                          <div className="flex gap-2">
                            <input
                              type="text"
                              value={bgImages[1]}
                        ...
                        <div>
                          ...
                          >
                            {uploading ? 'Upload en cours...' : '📁 Choisir l\'image de droite'}
                          </label>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-3 bg-slate-800/30 rounded-lg border border-slate-700">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          onChange={(e) => setLayoutType(e.target.checked ? 'single' : 'split')}
